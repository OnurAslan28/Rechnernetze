...
public class RFTClient extends Thread {
    ...
    public void computeTimeoutInterval(long sampleRTT) {
        ...
        if (estimatedRTT == -1) {
            estimatedRTT = sampleRTT * 4.0;
        } else {
            estimatedRTT = (long) ((1.0 - X) * estimatedRTT + (X * sampleRTT));
        }
        deviation = (long) ((1.0 - X) * deviation + (X * Math.abs(sampleRTT - estimatedRTT)));
        timeoutInterval = (long) (estimatedRTT + (4.0 * deviation));
